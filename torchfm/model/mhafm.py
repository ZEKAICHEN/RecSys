...
from torchfm.layer import FeaturesEmbedding, FeaturesLinear, MultiheadAttentionalFactorizationMachine, MultiLayerPerceptron
...
class MultiheadAttentionalFactorizationMachineModel(torch.nn.Module):
    ...
    def __init__(self, field_dims, embed_dim, num_heads, ffn_embed_dim, num_layers, mlp_dims, dropout):
        ...
        self.mhafm = MultiheadAttentionalFactorizationMachine(self.num_fields, embed_dim, num_heads, ffn_embed_dim, num_layers, dropout)
    ...
    def forward(self, x):
        ...
        x = self.linear(x) + self.mhafm(embed_x, attn_mask) + self.mlp(embed_x.view(-1, self.embed_output_dim))
